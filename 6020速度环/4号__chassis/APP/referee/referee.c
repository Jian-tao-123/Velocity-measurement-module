...
//        }
        ...
        break;
				...
				{
						memcpy(&robot_command, frame + index, sizeof(robot_command));
				}
				break;
        default:
        {
...
//------------------------3.15-----------------//
void get_game_process_get_time(uint8_t *game_process,	uint16_t*time)
{
    *game_process = game_state.game_progress;
    *time = game_state. stage_remain_time;
}
...
void get_self_own_side_blood(uint16_t*outpost_blood,uint16_t*self_blood,uint16_t*enemy_outpost_blood)
{
	...
	{
		*outpost_blood= game_robot_HP_t.red_outpost_HP;
		*self_blood=game_robot_HP_t.red_7_robot_HP;
		*enemy_outpost_blood=game_robot_HP_t.blue_outpost_HP;
	}
	if(robot_state.robot_id==107)
	{
		*outpost_blood=game_robot_HP_t.blue_outpost_HP;
		*self_blood=game_robot_HP_t.blue_7_robot_HP;
		*enemy_outpost_blood=game_robot_HP_t.red_outpost_HP;

	}	
}
void get_robot_hurt( uint8_t* shot_flag)
{if(robot_hurt.armor_id!=0)
	{*shot_flag=1;
	}
	if(robot_hurt.armor_id==0)
	{*shot_flag=0;
...
void get_game_state(uint8_t* game_process)
{
	*game_process=game_state.game_progress;
}
//------------------------5.26-----------------//
...
{
	*x=game_robot_pos_t.x;
	*y=game_robot_pos_t.y;
	*z=game_robot_pos_t.z;
	*yaw=game_robot_pos_t.yaw;
}
//ÉÚ±øÓÃ
void get_referee_warning(uint8_t *level,uint8_t *foul_robot_id)
{
	*foul_robot_id=referee_warning_t.foul_robot_id;
	if(referee_warning_t.foul_robot_id==7 || referee_warning_t.foul_robot_id==107)
		*level=referee_warning_t.level;
	else
		*level=0;
}
void get_robot_target_position(float *target_position_x,float *target_position_y,uint8_t *commd_keyboard)
{
	*target_position_x=robot_command.target_position_x;
	*target_position_y=robot_command.target_position_y;
	*commd_keyboard=robot_command.commd_keyboard;
}
